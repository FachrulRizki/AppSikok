        ...
        <div class="card-body">
            <div class="row mb-3">
                @php
                    ...
                    $defaultWaktu = old(
                        'waktu',
                        isset($aktivitas_keperawatan) && $aktivitas_keperawatan->waktu
                            ? $aktivitas_keperawatan->waktu->format('Y-m-d\TH:i')
            ...
            <div class="mb-4">
                ...
                <p class="text-danger">
                    <strong>Note.</strong> Selain Menambahkan Aktivitas Yang Tidak Ada di Dalam Kolom Aktivitas Keperawatan,
                    Tambahkan Juga Jadwal Dinas (Shift Malam, 19 Juni 2025)
                </p>
                        ...
                        <tr v-for="(act, actIdx) in activities" :key="act.id">
                            <td>
                                <div class="form-check">
                                    <input type="checkbox" :id="'activity_' + act.id" :value="act.id"
                                        :checked="selectedActivities.includes(act.id)" @change="toggleActivity(act)"
                                        class="form-check-input">
                                    <label :for="'activity_' + act.id" class="form-check-label">@{{ act.kode }}.
                                        @{{ act.nama }}</label>
                                </div>
                            </td>
                            <td>
                                <div v-if="selectedActivities.includes(act.id)">
                                    ...
                                    <div v-for="detail in act.activity_details" :key="detail.id"
                                        class="form-check">
                                        <input type="checkbox" :id="'detail_' + detail.id" :value="detail.id"
                                            :checked="selectedDetails[act.id]?.includes(detail.id)"
                                            @change="toggleDetail(detail, act.id)" class="form-check-input">
                                        <label :for="'detail_' + detail.id"
                                            class="form-check-label">@{{ detail.kode }}.
                                            @{{ detail.nama }}</label>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div v-for="detail in act.activity_details.filter(d => selectedDetails[act.id]?.includes(d.id))"
                                    :key="detail.id" class="mb-3">
                                    <div class="mb-2 fw-semibold">@{{ detail.kode }}. @{{ detail.nama }}</div>
                                    <div v-for="(groupedTasks, tipe) in groupTasks(detail.activity_tasks)"
                                        :key="tipe" class="mb-2">
                                        <div class="form-check">
                                            <input type="checkbox" :checked="isGroupChecked(detail.id, groupedTasks)"
                                                @change="toggleGroup(detail.id, groupedTasks, $event)"
                                                class="form-check-input">
                                            <label class="form-check-label fw-semibold">@{{ tipe }} :</label>
                                        </div>
                                        <div v-for="task in groupedTasks" :key="task.id"
                                            ...
                                            <label :for="'task_' + task.id"
                                                class="form-check-label">@{{ task.nama }}</label>
                                        </div>
                                    </div>
                                </div>
                            </td>
    ...
    createApp({
        ...
        methods: {
            ...
            toggleDetail(detail, activityId) {
                ...
                if (!this.selectedDetails[activityId] || !this.selectedDetails[activityId].includes(detail
                        .id)) {
